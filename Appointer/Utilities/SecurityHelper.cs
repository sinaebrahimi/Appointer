...
namespace Appointer.Utility
{
    ...
    {/// <summary>
     ...
     /// <returns>Input string if its lengh is OK; otherwise, truncated input string</returns>
        ...
        {
            return str.DeleteStrs("--", "/ava", "/Ava", "waitfor", "windows/win.ini", "win.ini", "sleep(", "acunetix",
                "print(", "md5(", "die(", "&dir", "|dir", "alert");
        }
        public static string GetErrors(this IEnumerable<KeyValuePair<string, ModelState>> modelState)
        {
            return (from item in modelState
                    where item.Value.Errors.Any()
                    select item.Value.Errors[0].ErrorMessage).ToList().Join("<br />");

            //return modelState.Values.SelectMany(m => m.Errors).Select(e => e.ErrorMessage).ToList().Join("<br />");
            //return (from item in modelState.Values
            //        from error in item.Errors
            //        select error.ErrorMessage).ToList().Join("<br />");
        }

        //public static string GetErrors(this IEnumerable<KeyValuePair<string, System.Web.Http.ModelBinding.ModelState>> modelState)
        //{
        //    return (from item in modelState
        //            where item.Value.Errors.Any()
        //            select item.Value.Errors[0].ErrorMessage).ToList().Join("<br />");
        ...
        private static void AddError(ref string innerEx, Exception ex, HttpContext context)
        {
            innerEx += "\"" + ex.GetType().FullName + " : " + ex.Message + "\"";
            innerEx += "\n----------------------------------------\n";
            innerEx += ex.StackTrace;
            innerEx += "\n----------------------------------------\n";
            innerEx += "Url : " + context.Request.Url + "\nRaw Url : " + context.Request.RawUrl;
            if (ex.InnerException != null)
            {
                innerEx += "\n----------------------------------------\n";
                AddError(ref innerEx, ex.InnerException, context);
                innerEx += "\n\n";
            }
        }
        ...
        public static string GetErrorDetail(this Exception ex)
        {
            ...
            {
                var desc = "";
                AddError(ref desc, ex, HttpContext.Current);
                ...
                text += "InsertDate : " + DateTime.Now + "\n";
                text += "Page : " + frame.GetFileName() + "\n";
                text += "LineNumber : " + frame.GetFileLineNumber() + "\n";
                text += "ColumnNumber : " + frame.GetFileColumnNumber() + "\n";
                text += "MethodName : " + frame.GetMethod().Name + "\n";
                text += "===========================================\n";
                return text;
            }
            catch
